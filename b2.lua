...
local menus = {}
local keys = {up = 172, down = 173, left = 174, right = 175, select = 191, back = 177}
local optionCount = 0
	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "Ciastekbatak pozdrawia")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Ciastekbatak Pozdrawia najlepszy H4CKER JEBAC ARIESA")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "BRUTAN LEAKED BY CIASTEKBATAK", 10000000)
				end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.CreateMenu("MainMenu", "CiastekMenu ~n~~r~V8")
		TiagoMenu.SetSubTitle("MainMenu", "Leaked by Ciastekbatak")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working Brutan Leak by Ciastekbatak")
				drawNotification("~g~Leaked by Ciastekbatak")
			...
			elseif TiagoMenu.IsMenuOpened("SelfMenu") then
				...
				elseif TiagoMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailerinos:unjail', -1)
	...
	then
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				...
				TriggerServerEvent("esx_jailerinos:unjail", GetPlayerServerId(SelectedPlayer), 0)
				...
				elseif TiagoMenu.Button("~b~Fuck Car ~r~NEW ") then
					...
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
						...
						SetVehicleNumberPlateText(playerVeh, "Cookie")
