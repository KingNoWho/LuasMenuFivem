...
NacroxMenu.CreateMenu(id, title)
...
NacroxMenu.CreateSubMenu(id, parent, subTitle)
...
NacroxMenu.CurrentMenu()
	return currentMenu
end

functio

NacroxMenu.OpenMenu(id)
	if id and menus[id] then
...
NacroxMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

functio

NacroxMenu.IsAnyMenuOpened()
...
NacroxMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
NacroxMenu.Button(text, subText)
...
NacroxMenu.MenuButton(text, id)
	if menus[id] then
		i
		
	
	NacroxMenu..Button(text) then
			setMenuVisible(currentMenu, false)
...
NacroxMenu.CheckBox(text, bool, callback)
...
NacroxMenu..Button(text, checked) then
		bool = not bool
...
NacroxMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
...
NacroxMenu..Button(text, selectedItem) then
...
NacroxMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

functio

NacroxMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

functio

NacroxMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

functio

NacroxMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

functio

NacroxMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

functio

NacroxMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
NacroxMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

functio

NacroxMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

functio

NacroxMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
NacroxMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

functio

NacroxMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

functio

NacroxMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

functio

NacroxMenu.SetMenuButtonPressedSound(id, name, set)
	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " BEM VINDOS *NACROXMODZ* https://discord.gg/MRnVqVR XD ")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Nacrox Menu", 10000000)
				end
			end
...
NacroxMenu.	.CreateMenu("MainMenu", "Nacrox ~n~~r~MENU")


NacroxMenu.	.SetSubTitle("MainMenu", "Creator NacroxModz")
...
NacroxMenu.CreateSubMenu("ESXMoney", "ServerMenu", "ESX Money Options")
...
NacroxMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")
		...
		NacroxMenu..IsMenuOpened("MainMenu") then
				...
				NacroxMenu.  .MenuButton("~h~Player ~s~Menu                                       ~r~      »", "SelfMenu") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~Online ~s~Players                                   ~r~       »", "OnlinePlayerMenu") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~World ~s~Menu                                      ~r~        »", "World") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~Teleporte ~s~Menu                                  ~r~      »", "TeleportMenu") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~Vehicles ~s~Menu                                  ~r~        »", "VehMenu") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~Weapons ~s~Menu                                  ~r~      »", "WepMenu") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~Fuck ~s~Server                                   ~r~           »", "Fuck") then
				elsei
				
			
			NacroxMenu..MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~~r~»", "ServerMenu") then
				elsei
				
			
			NacroxMenu..Button("~h~~r~Disconnect ~s~Menu") then
					Enabled = false
				end
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("SelfMenu") then
				if
		...
		NacroxMenu.	.CheckBox(
			...
			NacroxMenu..Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elsei
				
			
			NacroxMenu..Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				elsei
				
			
			NacroxMenu..Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elsei
				
			
			NacroxMenu..Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elsei
				
			
			NacroxMenu..Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elsei
				
			
			NacroxMenu..Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elsei
				
			
			NacroxMenu..Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elsei
				
			
			NacroxMenu..Button("~h~~b~Unjail") then
	...
	NacroxMenu.		.CheckBox(
			...
			NacroxMenu..CheckBox("~h~Speed Demon",SpeedDemon,
...
NacroxMenu.			.Display()
		...
		NacroxMenu..IsMenuOpened("TeleportMenu") then
				i
				
			
			NacroxMenu..Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elsei
			 
			
			NacroxMenu..Button("~h~Teleport ~h~~g~Vehicle") then
...
NacroxMenu.		 .Display()
		...
		NacroxMenu..IsMenuOpened("World") then
			if
...
NacroxMenu.		.Display()
	...
	NacroxMenu..IsMenuOpened("Fuck") then
		i
		
	
	NacroxMenu..Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
			...
			NacroxMenu..Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
...
NacroxMenu..Display()
		...
		NacroxMenu..IsMenuOpened("WepMenu") then
				i
				
			
			NacroxMenu..MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elsei
				
			
			NacroxMenu..Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
			...
			NacroxMenu..Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
			...
			NacroxMenu..Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					i
					
				
				NacroxMenu..Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("VehMenu") then
				i
				
			
			NacroxMenu..Button("~h~~g~Spawn Vehicle") then
			...
			NacroxMenu..Button("~h~Repair Vehicle") then
			...
			NacroxMenu..Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elsei

NacroxMenu.Button("~h~Max mechanics ~r~new") then
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("ServerMenu") then
				i
				
			
			NacroxMenu..MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elsei
				
			
			NacroxMenu..MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elsei
				
			
			NacroxMenu..MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elsei
				
			
			NacroxMenu..MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elsei
				
			
			NacroxMenu..MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("ESXMoney") then
				i
				
			
			NacroxMenu..Button("~g~~h~NEW ~r~~h~BEST MONEY") then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~1") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~2") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~3") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~4") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~5") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~6") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~7") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~8") then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~9") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~10") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~11") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~12") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~13") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~14") then
		...
		NacroxMenu..Button("~g~~h~MONEY ~s~15") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
		...
		NacroxMenu..Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
		...
		NacroxMenu..Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
...
NacroxMenu.		.Display()
			...
			NacroxMenu..IsMenuOpened("ESXMisc") then
					i
					
				
				NacroxMenu..Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
			...
			NacroxMenu..Button("~g~~h~ESX ~w~Get all licenses Drive") then
			...
			NacroxMenu..Button("~h~Send Discord Message") then
			...
			NacroxMenu..Button("~h~Send Police Car Advert") then
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("MiscServerOptions") then
				i
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("VRPOptions") then
				i
				
			
			NacroxMenu..Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
			...
			NacroxMenu..Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
			...
			NacroxMenu..Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elsei
				
			
			NacroxMenu..Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
			...
			NacroxMenu..Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
	...
	NacroxMenu.	.Display()
		...
		NacroxMenu..IsMenuOpened("ESXDrugs") then
				i
				
			
			NacroxMenu..Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~r~Harvest Meth ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~r~Transform Meth ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~r~Sell Meth ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~p~Harvest Opium ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~p~Transform Opium ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~p~Sell Opium ~c~(x5)") then
			...
			NacroxMenu..Button("~h~~g~Money Wash ~c~(x10)") then
			...
			NacroxMenu..Button("~r~~h~Stop all ~c~(Drugs)") then
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 an
					
				
				NacroxMenu..MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("RecrutarPlayersOptions") then
	...
	NacroxMenu.	.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			...
			NacroxMenu..Button("~s~Recruit~c~~h~ Mechanic") then
		...
		NacroxMenu..Button("~s~Recruit~b~~h~ Police") then
		...
		NacroxMenu..Button("~s~Recruit~c~~h~ Mafia") then
		...
		NacroxMenu..Button("~s~Recruit~p~~h~ Gang") then
		...
		NacroxMenu..Button("~s~Recruit~r~~h~ Inem") then
		...
		NacroxMenu..Button("~h~Custom Recruit") then
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 an
					
				
				NacroxMenu..MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
	...
	NacroxMenu.		.Display()
		...
		NacroxMenu..IsMenuOpened("PlayerOptionsMenu") then
		
		
	
	NacroxMenu.	.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~[" .. GetPlayerName(SelectedPlayer) .. "]")
				i
				
			
			NacroxMenu..Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elsei
				
			
			NacroxMenu..Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elsei
				
			
			NacroxMenu..Button("~g~Give ~s~Money") then
			...
			NacroxMenu..Button("~r~Remove ~s~Money") then
						local result = KeyboardInput("Enter amount of money to give", "", 100000000)
						if result then
			...
			NacroxMenu..Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elsei
					
				
				NacroxMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elsei
					
				
				NacroxMenu..Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elsei
				
			
			NacroxMenu..Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elsei
					
				
				NacroxMenu..Button("Jail") then		
			...
			NacroxMenu..Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
			...
			NacroxMenu..MenuButton("Weapon Menu", "SingleWepPlayer") then
				elsei
				
			
			NacroxMenu..Button("Spawn Vehicle") then
	...
	NacroxMenu.			.Display()
		...
		NacroxMenu..IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					i
