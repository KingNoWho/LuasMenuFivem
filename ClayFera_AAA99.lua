...
local function debugPrint(text)
    if EuropeuMenu.debug then
        Citizen.Trace('[EuropeuMenu] ' .. tostring(text))
    end
end
...
function EuropeuMenu.CreateMenu(id, title)
    menus[id] = {}
...
function EuropeuMenu.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        EuropeuMenu.CreateMenu(id, menus[parent].title)
        if subTitle then
...
function EuropeuMenu.CurrentMenu()
    return currentMenu
end

function EuropeuMenu.OpenMenu(id)
    if id and menus[id] then
...
function EuropeuMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end

function EuropeuMenu.IsAnyMenuOpened()
...
function EuropeuMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function EuropeuMenu.CloseMenu()
    if menus[currentMenu] then
...
function EuropeuMenu.Button(text, subText)
...
function EuropeuMenu.MenuButton(text, id)
    if menus[id] then
        if EuropeuMenu.Button(text .. themecolor .. "   " .. themearrow) then
            setMenuVisible(currentMenu, arwet)
...
function EuropeuMenu.CheckBox(text, checked, offtext, ontext, callback)
    if not offtext then
        offtext = "Off"
    end
    ...
    if EuropeuMenu.Button(text, checked and ontext or offtext) then
        checked = not checked
...
function EuropeuMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if EuropeuMenu.Button(text, selectedItem) then
...
function EuropeuMenu.Display()
    if isMenuVisible(currentMenu) then
        if menus[currentMenu].aboutToBeClosed then
            EuropeuMenu.CloseMenu()
        else
...
function EuropeuMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end

function EuropeuMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end

function EuropeuMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end

function EuropeuMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function EuropeuMenu.SetTitle(id, title)
    setMenuProperty(id, 'title', title)
end

function EuropeuMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleColor.a})
end

function EuropeuMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleBackgroundColor.a})
end

function EuropeuMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
    RequestStreamedTextureDict(textureDict)
    setMenuProperty(id, 'titleBackgroundSprite', {dict = textureDict, name = textureName})
end

function EuropeuMenu.SetSubTitle(id, text)
    setMenuProperty(id, 'subTitle', string.upper(text))
end

function EuropeuMenu.SetMenuBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'menuBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuBackgroundColor.a})
end

function EuropeuMenu.SetMenuTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuTextColor.a})
end

function EuropeuMenu.SetMenuSubTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuSubTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuSubTextColor.a})
end

function EuropeuMenu.SetMenuFocusColor(id, r, g, b, a)
    setMenuProperty(id, 'menuFocusColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuFocusColor.a})
end

function EuropeuMenu.SetMenuButtonPressedSound(id, name, set)
...
version = ""
theme = "infamous"
themes = {"infamous", "basic", "dark", "EuropeuMenu"}
mpMessage = arwet
menuKeybind = "PAGEUP"
menuKeybind2 = "PAGEUP"
noclipKeybind = "F6"
teleportKeyblind = "F2"
startMessage = "EuropeuMenu ~n~~r~Injected Complete"
...
function EuropeuMenu.SetFont(id, font)
    buttonFont = font
    menus[id].titleFont = font
end

function EuropeuMenu.SetMenuFocusBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, "menuFocusBackgroundColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusBackgroundColor.a})
end

function EuropeuMenu.SetMaxOptionCount(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function EuropeuMenu.PopupWindow(x, y, title)

end


function EuropeuMenu.SetTheme(id, theme)
    if theme == "basic" then
        EuropeuMenu.SetMenuBackgroundColor(id, 81, 231, 251, 125)
        EuropeuMenu.SetTitleBackgroundColor(id, 92, 212, 249, 80)
        EuropeuMenu.SetTitleColor(id, 92, 212, 249, 230)
        EuropeuMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        EuropeuMenu.SetMenuFocusColor(id, 40, 40, 40, 230)
        EuropeuMenu.SetFont(id, 7)
        EuropeuMenu.SetMenuX(id, .75)-- [0.0..1.0] top left corner
        EuropeuMenu.SetMenuY(id, .1)-- [0.0..1.0] top
        EuropeuMenu.SetMenuWidth(id, 0.23)-- 0.23
        EuropeuMenu.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "dark" then
        EuropeuMenu.SetMenuBackgroundColor(id, 0, 153, 255, 255)
        EuropeuMenu.SetTitleBackgroundColor(id, 0, 153, 255, 255)
        EuropeuMenu.SetTitleColor(id, 0, 153, 255, 230)
        EuropeuMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        EuropeuMenu.SetMenuFocusColor(id, 40, 40, 40, 255)
        EuropeuMenu.SetFont(id, 1)
        EuropeuMenu.SetMenuX(id, .75)
        EuropeuMenu.SetMenuY(id, .1)
        EuropeuMenu.SetMenuWidth(id, 0.23)-- 0.23
        EuropeuMenu.SetMaxOptionCount(id, 12)-- 10
        ...
        themecolor = '~r~'
        themearrow = ">"
    elseif theme == "EuropeuMenu" then
        EuropeuMenu.SetMenuBackgroundColor(id, 50, 50, 160, 85)
        EuropeuMenu.SetTitleBackgroundColor(id, 92, 255, 0, 255)
        EuropeuMenu.SetTitleColor(id, 92, 212, 249, 170)
        EuropeuMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        EuropeuMenu.SetFont(id, 0)
        EuropeuMenu.SetMenuX(id, .75)
        EuropeuMenu.SetMenuY(id, .1)
        EuropeuMenu.SetMenuWidth(id, 0.23)-- 0.23
        EuropeuMenu.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "infamous" then
        EuropeuMenu.SetMenuBackgroundColor(id, 38, 38, 38, 255)
        EuropeuMenu.SetTitleBackgroundColor(id, 28, 108, 237, 255)
        EuropeuMenu.SetTitleColor(id, 28, 108, 237, 255)
        EuropeuMenu.SetMenuSubTextColor(id, 28, 108, 237, 255)
        EuropeuMenu.SetMenuFocusBackgroundColor(id, 38, 39, 110, 255)
        EuropeuMenu.SetFont(id, 4)
        EuropeuMenu.SetMenuX(id, .725)
        EuropeuMenu.SetMenuY(id, .1)
        EuropeuMenu.SetMenuWidth(id, 0.25)-- 0.23
        EuropeuMenu.SetMaxOptionCount(id, 13)-- 10
...
function EuropeuMenu.InitializeTheme()
    for i = 1, #menulist do
        EuropeuMenu.SetTheme(menulist[i], theme)
    end
end

-- ComboBox w/ new index behaviour (does not wrap around)
function EuropeuMenu.ComboBox2(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
...
function EuropeuMenu.ComboBoxSlider(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if EuropeuMenu.Button2(text, items, itemsCount, currentIndex) then
		selectedIndex = currentIndex
		callback(currentIndex, selectedIndex)
		return true
	elseif isCurrent then
...
function EuropeuMenu.Button2(text, items, itemsCount, currentIndex)
	local buttonText = text
...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "EuropeuMenu" then -- static effect for EuropeuMenu theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            if p == 5 then p = 1 else p = p + 1 end
            for i = 1, #menulist do
                if EuropeuMenu.IsMenuOpened(menulist[i]) then EuropeuMenu.SetTitleBackgroundSprite(menulist[i], 'digitaloverlay', 'static' .. p) end
            end
        else -- Base textures
            for i = 1, #menulist do EuropeuMenu.SetTitleBackgroundSprite(menulist[i], 'mpinventory', 'darts') end
        end
        Wait(100)
    end
end)
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    EuropeuMenu.CreateMenu('EuropeuMenu', menuName .. '' .. version)
    EuropeuMenu.SetSubTitle('EuropeuMenu', '~s~                                                         EuropeuMenu')

    -- MAIN MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('\112\108\97\121\101\114\10', 'EuropeuMenu', 'Player Options')
    EuropeuMenu.CreateSubMenu('\115\101\108\102\10', 'EuropeuMenu', 'Self Options')
    EuropeuMenu.CreateSubMenu('\119\101\97\112\111\110\10', 'EuropeuMenu', 'Weapon Options')
    EuropeuMenu.CreateSubMenu('\118\101\104\105\99\108\101\10', 'EuropeuMenu', 'Vehicle Options')
    EuropeuMenu.CreateSubMenu('\119\111\114\108\100\10', 'EuropeuMenu', 'World Options')
	EuropeuMenu.CreateSubMenu('\116\101\108\101\112\111\114\116\10', 'EuropeuMenu', 'Teleport Options')
    EuropeuMenu.CreateSubMenu('\109\105\115\99\10', 'EuropeuMenu', 'Misc Options')
    EuropeuMenu.CreateSubMenu('\108\117\97\10', 'EuropeuMenu', 'Lua Options')
    EuropeuMenu.CreateSubMenu('\115\101\116\116\105\110\103\115\10', 'EuropeuMenu', '\115\101\116\116\105\110\103\115\10')
    EuropeuMenu.CreateSubMenu('\108\121\110\120\10', 'EuropeuMenu', '\108\121\110\120\10 bro....')
    -- PLAYER MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('allplayer', '\112\108\97\121\101\114\10', 'All Players')
    EuropeuMenu.CreateSubMenu('\112\108\97\121\101\114\111\112\116\105\111\110\115\10', '\112\108\97\121\101\114\10', 'Player Options')
    EuropeuMenu.CreateSubMenu('troll', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Troll Options')
    EuropeuMenu.CreateSubMenu('jobsplayers', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')
    EuropeuMenu.CreateSubMenu('jobsplayers2', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')


    -- SELF MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('\97\112\112\101\97\114\97\110\99\101\10', 'EuropeuMenu', 'Appearance Options')
    EuropeuMenu.CreateSubMenu('modifiers', '\115\101\108\102\10', 'Modifiers Options')

	-- APPEARANCE SUBMENUS
	EuropeuMenu.CreateSubMenu('modifyskintextures', '\97\112\112\101\97\114\97\110\99\101\10', "Modify Skin Textures")
    EuropeuMenu.CreateSubMenu('modifyhead', 'modifyskintextures', "Available Drawables")
	EuropeuMenu.CreateSubMenu('skinsmodels', '\97\112\112\101\97\114\97\110\99\101\10', "Skin Models")

    -- WEAPON MENU SUBMENUS

	EuropeuMenu.CreateSubMenu('WeaponCustomization', '\119\101\97\112\111\110\10', 'Weapon Customization')
    EuropeuMenu.CreateSubMenu('\119\101\97\112\111\110\115\112\97\119\110\101\114\10', '\119\101\97\112\111\110\10', 'Weapon Spawner')
    EuropeuMenu.CreateSubMenu('melee', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Melee Weapons')
    EuropeuMenu.CreateSubMenu('pistol', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Pistols')
    EuropeuMenu.CreateSubMenu('smg', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'SMGs / MGs')
    EuropeuMenu.CreateSubMenu('shotgun', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Shotguns')
    EuropeuMenu.CreateSubMenu('assault', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Assault Rifles')
    EuropeuMenu.CreateSubMenu('sniper', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Sniper Rifles')
    EuropeuMenu.CreateSubMenu('thrown', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Thrown Weapons')
    EuropeuMenu.CreateSubMenu('heavy', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Heavy Weapons')

    -- VEHICLE MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', '\118\101\104\105\99\108\101\10', 'Vehicle Spawner')
    EuropeuMenu.CreateSubMenu('vehiclemods', '\118\101\104\105\99\108\101\10', 'Vehicle Mods')
    EuropeuMenu.CreateSubMenu('vehiclemenu', '\118\101\104\105\99\108\101\10', 'Vehicle Control Menu')

    -- VEHICLE SPAWNER MENU
    EuropeuMenu.CreateSubMenu('compacts', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Compacts')
    EuropeuMenu.CreateSubMenu('sedans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sedans')
    EuropeuMenu.CreateSubMenu('suvs', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'SUVs')
    EuropeuMenu.CreateSubMenu('coupes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Coupes')
    EuropeuMenu.CreateSubMenu('muscle', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Muscle')
    EuropeuMenu.CreateSubMenu('sportsclassics', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports Classics')
    EuropeuMenu.CreateSubMenu('sports', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports')
    EuropeuMenu.CreateSubMenu('super', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Super')
    EuropeuMenu.CreateSubMenu('motorcycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Motorcycles')
    EuropeuMenu.CreateSubMenu('offroad', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Off-Road')
    EuropeuMenu.CreateSubMenu('industrial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Industrial')
    EuropeuMenu.CreateSubMenu('utility', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Utility')
    EuropeuMenu.CreateSubMenu('vans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Vans')
    EuropeuMenu.CreateSubMenu('cycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Cycles')
    EuropeuMenu.CreateSubMenu('boats', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Boats')
    EuropeuMenu.CreateSubMenu('helicopters', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Helicopters')
    EuropeuMenu.CreateSubMenu('planes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Planes')
    EuropeuMenu.CreateSubMenu('service', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Service')
    EuropeuMenu.CreateSubMenu('commercial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Commercial')

    -- VEHICLE MODS SUBMENUS
    EuropeuMenu.CreateSubMenu('vehiclecolors', 'vehiclemods', 'Vehicle Colors')
    EuropeuMenu.CreateSubMenu('vehiclecolors_primary', 'vehiclecolors', 'Primary Color')
    EuropeuMenu.CreateSubMenu('vehiclecolors_secondary', 'vehiclecolors', 'Secondary Color')

    EuropeuMenu.CreateSubMenu('primary_classic', 'vehiclecolors_primary', 'Classic Colors')
    EuropeuMenu.CreateSubMenu('primary_matte', 'vehiclecolors_primary', 'Matte Colors')
    EuropeuMenu.CreateSubMenu('primary_metal', 'vehiclecolors_primary', 'Metals')

    EuropeuMenu.CreateSubMenu('secondary_classic', 'vehiclecolors_secondary', 'Classic Colors')
    EuropeuMenu.CreateSubMenu('secondary_matte', 'vehiclecolors_secondary', 'Matte Colors')
    EuropeuMenu.CreateSubMenu('secondary_metal', 'vehiclecolors_secondary', 'Metals')

    EuropeuMenu.CreateSubMenu('vehicletuning', 'vehiclemods', 'Vehicle Tuning')

    -- WORLD MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'EuropeuMenu', 'Object Spawner')
    EuropeuMenu.CreateSubMenu('objectlist', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'Select To Delete')
    EuropeuMenu.CreateSubMenu('weather', '\119\111\114\108\100\10', 'Weather Changer | ~r~Client Side')
    EuropeuMenu.CreateSubMenu('time', '\119\111\114\108\100\10', 'Time Changer')

    -- MISC MENU SUBMENUS
	EuropeuMenu.CreateSubMenu('esp', '\109\105\115\99\10', 'ESP & Visual Options')
	EuropeuMenu.CreateSubMenu('webradio', '\109\105\115\99\10', 'Web Radio')
    EuropeuMenu.CreateSubMenu('credits', '\109\105\115\99\10', 'Credits')
    EuropeuMenu.CreateSubMenu('InfoMenu', '\109\105\115\99\10', 'Info')

    -- TELEPORT MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('saveload', '\116\101\108\101\112\111\114\116\10', 'Tøjbutikker')
    EuropeuMenu.CreateSubMenu('pois', '\116\101\108\101\112\111\114\116\10', 'Garager')
    EuropeuMenu.CreateSubMenu('pois2', '\116\101\108\101\112\111\114\116\10', 'Steder')

	--fuck server
	EuropeuMenu.CreateSubMenu('\102\117\99\107\115\101\114\118\101\114\10', 'EuropeuMenu', 'Fuck Server')

    -- LUA MENU SUBMENUS
    EuropeuMenu.CreateSubMenu('\101\115\120\10', '\108\117\97\10', 'ESX Options')
    EuropeuMenu.CreateSubMenu('\118\114\112\10', '\108\117\97\10', 'vRP Options')
    EuropeuMenu.CreateSubMenu('roles', '\108\117\97\10', 'vRP Rolespilleren')
    EuropeuMenu.CreateSubMenu('other', '\108\117\97\10', 'Esx Jobs')
    EuropeuMenu.CreateSubMenu('money', '\118\114\112\10', 'Money Options')

    EuropeuMenu.InitializeTheme()


    while true do
        -- MAIN MENU
        
        if EuropeuMenu.IsMenuOpened('EuropeuMenu') then
        	if EuropeuMenu.MenuButton('Self-Options', '\115\101\108\102\10') then
            elseif EuropeuMenu.MenuButton('Online Players', '\112\108\97\121\101\114\10') then
            elseif EuropeuMenu.MenuButton('LUA', '\108\117\97\10') then
            elseif EuropeuMenu.MenuButton('Weapons', '\119\101\97\112\111\110\10') then
           	elseif EuropeuMenu.MenuButton('Vehicles', '\118\101\104\105\99\108\101\10') then
			elseif EuropeuMenu.MenuButton('Models', '\97\112\112\101\97\114\97\110\99\101\10') then
            elseif EuropeuMenu.MenuButton('World', '\119\111\114\108\100\10') then
			elseif EuropeuMenu.MenuButton('Teleport', '\116\101\108\101\112\111\114\116\10') then
            elseif EuropeuMenu.MenuButton('Misc', '\109\105\115\99\10') then
			elseif EuropeuMenu.MenuButton('Objects', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
			elseif EuropeuMenu.MenuButton('RIP Server', '\102\117\99\107\115\101\114\118\101\114\10') then
            elseif EuropeuMenu.MenuButton('Options', '\115\101\116\116\105\110\103\115\10') then
            elseif EuropeuMenu.Button('Close Menu') then break
            end

        -- PLAYER OPTIONS MENU
        elseif EuropeuMenu.IsMenuOpened('\112\108\97\121\101\114\10') then

                local playerlist = GetActivePlayers()
                for i = 1, #playerlist do
                    local currPlayer = playerlist[i]
                    if EuropeuMenu.MenuButton("ID: ~r~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer), '\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
		...
		--Fuck server
		elseif EuropeuMenu.IsMenuOpened('\102\117\99\107\115\101\114\118\101\114\10') then
		    if EuropeuMenu.CheckBox("~s~Include Self", includeself, "No", "Yes") then
                includeself = not includeself
            elseif EuropeuMenu.Button("~s~Fake ~s~Chat Message") then 
            ...
            elseif EuropeuMenu.Button("~s~Nuke Server") then
			ShowInfo("~y~Fucking Players...")
                nukeserver()
            elseif EuropeuMenu.Button("~s~Rape All") then
                RapeAllFunc() 
            elseif EuropeuMenu.Button("~s~Change ~s~All Nearby Vehicles Plate Text") then
            local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
            end
			elseif EuropeuMenu.CheckBox("~s~Make ~s~All Cars ~r~Fly", FlyingCars) then
                FlyingCars = not FlyingCars
            elseif EuropeuMenu.CheckBox("~s~Set ~s~The World On ~r~Fire", WorldOnFire) then
			...
			elseif EuropeuMenu.Button("~s~Close ~s~The Whole Square") then
	...
	elseif EuropeuMenu.Button("~s~SPAWN ~r~Lion ~s~On All Players") then
		...
		elseif EuropeuMenu.Button("~s~SPAWN ~r~SWAT ~s~On All Players ") then
	 ...
	 elseif EuropeuMenu.Button('~s~Make All Players A Hamburger Head') then
                    local plist = GetActivePlayers()
                        for i = 0, #plist do
            ...
            elseif EuropeuMenu.Button("~s~Fuck Up The Map | Irreversible! | ~r~[WIP]") then
                if not FuckMap then
            ...
            elseif EuropeuMenu.Button("~s~Explode All Players") then
                ExplodeAll(includeself)
            elseif EuropeuMenu.CheckBox("~s~Explode All Players | ~r~LOOP", ExplodingAll) then
                ExplodingAll = not ExplodingAll
            elseif EuropeuMenu.Button("~s~Give All Players Weapons") then
                GiveAllPlayersWeapons(includeself)
            elseif EuropeuMenu.Button("~s~Remove All Players Weapons") then
                StripAll(includeself)
            elseif EuropeuMenu.Button("~s~FUCK ALL Vehicles") then
            elseif EuropeuMenu.Button("~s~Rampinator") then
            ...
            elseif EuropeuMenu.Button("~s~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
            elseif EuropeuMenu.Button("Kick All Players From Vehicle") then
                KickAllFromVeh(includeself)
            end

        elseif EuropeuMenu.IsMenuOpened('jobsplayers2') then
        if EuropeuMenu.Button("Udløs Panik Knap", '~r~~r~Devo & Sky') then
        ...
        elseif EuropeuMenu.Button("Launch Players Vehicle") then
        ...
        elseif EuropeuMenu.ComboBox("Pop Players Vehicle Tire", {"Front Left", "Front Right", "Back Left", "Back Right", "All"}, currTireIndex, selTireIndex, function(currentIndex, selClothingIndex)
        ...
        elseif EuropeuMenu.Button("Slam Players Vehicle") then
            if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
                ShowInfo("~r~Player Not In Vehicle!")
            else
        ...
        elseif EuropeuMenu.ComboBox("Teleport Into Players Vehicle", {"Front Right", "Back Left", "Back Right"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
        ...
        elseif EuropeuMenu.CheckBox("Track Player", Tracking, "Tracking: Nobody", "Tracking: "..GetPlayerName(TrackedPlayer)) then
            Tracking = not Tracking
            TrackedPlayer = selectedPlayer
        elseif EuropeuMenu.CheckBox("Fling Player", FlingingPlayer, "Flinging: Nobody", "Flinging: "..GetPlayerName(FlingedPlayer)) then
			...
			elseif EuropeuMenu.IsMenuOpened('jobsplayers') then
			if EuropeuMenu.Button("~s~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
			elseif EuropeuMenu.Button("~s~Recruit~s~ Mechanic") then
			...
			elseif EuropeuMenu.Button("~s~Recruit~s~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~s~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~s~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~s~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~s~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			 ...
			 elseif EuropeuMenu.IsMenuOpened('troll') then
			 if EuropeuMenu.Button('~r~Attack ~s~Windmill') then
					...
					elseif EuropeuMenu.Button('~r~Attack ~s~Weed Plant') then
		 local bI = CreateObject(GetHashKey('Prop_weed_01'), 0, 0, 0, true, true, true)
			 ...
			 elseif EuropeuMenu.Button('~r~Attack ~s~Giant Orange') then
			 ...
			 elseif EuropeuMenu.Button('~r~Cage ~s~Player') then
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
                    roundz = tonumber(string.format('%.2f', z))
			...
			elseif EuropeuMenu.Button("~r~SPAWN ~s~SWAT ARMY WITH ~r~AK") then
			...
			elseif EuropeuMenu.Button("~r~SPAWN ~s~STRIPPER ARMY WITH ~r~RPG") then
			...
			elseif EuropeuMenu.Button("Nearby Peds Attack Player") then
                PedAttack(selectedPlayer, PedAttackType)
            elseif EuropeuMenu.ComboBox("Ped Attack Type", PedAttackOps, currAttackTypeIndex, selAttackTypeIndex, function(currentIndex, selectedIndex)
                currAttackTypeIndex = currentIndex
                selAttackTypeIndex = currentIndex
                PedAttackType = currentIndex
            end) then
			 elseif EuropeuMenu.Button("Possess Player Vehicle") then
                if Spectating then SpectatePlayer(selectedPlayer) end
                PossessVehicle(selectedPlayer)

            elseif EuropeuMenu.Button("Explode Player") then
                ExplodePlayer(selectedPlayer)
			elseif EuropeuMenu.Button("Silent Kill Player") then
				local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                AddExplosion(coords.x, coords.y, coords.z, 4, 0.1, 0, 1, 0.0)
			elseif EuropeuMenu.Button("Kick From Vehicle") then
                KickFromVeh(selectedPlayer)
				end


        -- SPECIFIC PLAYER OPTIONS
        elseif EuropeuMenu.IsMenuOpened('\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
            if EuropeuMenu.Button("~s~PLAYER: " .. "~r~[" .. GetPlayerServerId(selectedPlayer) .. "] ~s~" .. GetPlayerName(selectedPlayer)) then
			elseif EuropeuMenu.CheckBox("~s~Spectate ~s~Player", Spectating, "Spectating: ~r~OFF", "Spectating: "..GetPlayerName(SpectatedPlayer)) then
				Spectating = not Spectating
				SpectatePlayer(selectedPlayer)
				SpectatedPlayer = selectedPlayer
            elseif EuropeuMenu.MenuButton("Troll Menu", 'troll') then
            elseif EuropeuMenu.MenuButton("~r~Custom ~s~| Troll Menu", 'jobsplayers2') then
            elseif EuropeuMenu.MenuButton("~r~ESX ~s~| ~s~Jobs Menu | ~r~RISK", 'jobsplayers') then
			elseif EuropeuMenu.Button("~r~ESX ~s~| ~s~Revive | ~r~RISK") then
			...
			elseif EuropeuMenu.Button('~r~vRP ~s~ | ~s~Revive') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif EuropeuMenu.Button('~r~Both ~s~| ~s~Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    ...
                    SetPickupRegenerationTime(pickup, 60)
            elseif EuropeuMenu.Button('~r~Both ~s~| ~s~Full Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    for i = 0, 99 do
                        Citizen.Wait(0)
            ...
            elseif EuropeuMenu.Button("Cancel Animation/Task") then
                ClearPedTasksImmediately(GetPlayerPed(selectedPlayer))
            elseif EuropeuMenu.Button("Give All Weapons") then
            GiveAllWeapons(selectedPlayer)
            elseif EuropeuMenu.Button("Remove All Weapons") then
                StripPlayer(selectedPlayer)
            end


        -- SELF OPTIONS MENU
        elseif EuropeuMenu.IsMenuOpened('\115\101\108\102\10') then
            if EuropeuMenu.CheckBox("~s~God~s~Mode", Godmode) then
                Godmode = not Godmode
                ToggleGodmode(Godmode)
            elseif EuropeuMenu.CheckBox("Demi-God Mode", Demigod) then
                Demigod = not Demigod
			elseif EuropeuMenu.Button("~r~ESX | ~s~Revive | ~r~RISK") then
				local confirm = GetKeyboardInput("Using This Option Will ~r~Risk Banned ~s~Server! Are you Sure? ~r~Y~w~/~r~N")
			 if string.lower(confirm) == "y" then
            ...
            elseif EuropeuMenu.Button('~r~ESX | ~s~Revive Friend | ~r~RISK') then
            ...
            elseif EuropeuMenu.ComboBox("~s~Player ~s~Functions", {"~r~vRP ~s~Revive", "~s~Give Player Armor", "~s~Remove Player Armor", "~s~Clean Player", "~s~Suicide", "~s~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
            ...
            elseif EuropeuMenu.ComboBoxSlider("~s~Fast Run", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selClothingIndex)
			...
			elseif EuropeuMenu.ComboBoxSlider("~s~Fast Swim", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selClothingIndex)
            ...
            elseif EuropeuMenu.CheckBox("Super Jump", SuperJump) then
                SuperJump = not SuperJump
            elseif EuropeuMenu.CheckBox("Invisibility", Invisibility) then
                Invisibility = not Invisibility
                if not Invisibility then
                    SetEntityVisible(PlayerPedId(), true)
                end
            elseif EuropeuMenu.CheckBox("~s~Magneto Mode ~s~KEY | ~r~[E]", ForceTog) then
                ForceMod()
            elseif EuropeuMenu.CheckBox("~s~Forcefield", Forcefield) then
                Forcefield = not Forcefield
			elseif EuropeuMenu.ComboBox("~s~Forcefield Radius | ~r~CHANGEABLE", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
            ...
            elseif EuropeuMenu.CheckBox("~s~Noclip", Noclipping) then
                ToggleNoclip()
			elseif EuropeuMenu.ComboBox("~s~Noclip Speed | ~r~CHANGEABLE", NoclipSpeedOps, currNoclipSpeedIndex, selNoclipSpeedIndex, function(currentIndex, selectedIndex)
        ...
        -- APPEARANCE MENU
        elseif EuropeuMenu.IsMenuOpened('\97\112\112\101\97\114\97\110\99\101\10') then
		if EuropeuMenu.MenuButton("Models", 'skinsmodels') then
            elseif EuropeuMenu.Button("Set Model Name") then
            ...
            elseif EuropeuMenu.MenuButton("Modify Skin Textures", 'modifyskintextures') then
            elseif EuropeuMenu.Button("~s~Randomize ~s~Model") then
                RandomClothes(PlayerId())
            elseif EuropeuMenu.ComboBox("Save Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
                Outfits[selClothingIndex] = GetCurrentOutfit(PlayerId())
            elseif EuropeuMenu.ComboBox("Load Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
            ...
            elseif EuropeuMenu.IsMenuOpened('modifyskintextures') then
                ...
                if EuropeuMenu.MenuButton("Head", "modifyhead") then

					if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
						EuropeuMenu.CloseMenu()
						EuropeuMenu.OpenMenu('modifyskintextures')
				...
				elseif EuropeuMenu.IsMenuOpened('skinsmodels') then

if EuropeuMenu.Button("~s~Reset Model To FiveM Player") then
	...
	elseif EuropeuMenu.Button("Change To ~r~Clown") then
			...
			elseif EuropeuMenu.Button("Change To ~r~Ortega") then
			local model = "csb_ortega"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif EuropeuMenu.Button("Change To ~r~Stranger") then
			local model = "csb_ramp_hipster"
				...
				if HasModelLoaded(GetHashKey(model)) then
                    ...
                    elseif EuropeuMenu.Button("Change To ~r~Ramp Gang") then    
			local model = "csb_ramp_mex"
                ...
                elseif EuropeuMenu.Button("Change To ~r~Jew") then
                    local model = "a_m_y_hasjew_01"
                        ...
                        elseif EuropeuMenu.Button("Change To ~r~Alien") then
                            ...
                            elseif EuropeuMenu.Button("Change To ~r~Monkey") then
                            local model ="a_c_chimp"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif EuropeuMenu.Button("Change To ~r~Slaughter") then
                            local model ="ig_chef2"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~S_Lausen") then
                            local model ="u_m_y_imporage"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif EuropeuMenu.Button("Change To ~r~Rollespilleren") then
                            local model ="u_m_y_babyd"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~Blackops") then
                            local model ="s_m_y_blackops_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif EuropeuMenu.Button("Change To ~r~Jesus") then
                            local model ="u_m_m_jesus_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~Pappa") then
                            local model ="mp_m_weapexp_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~Motherfucker") then
                            local model ="u_m_y_juggernaut_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~Mexicano") then
                            local model ="u_m_y_mani"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif EuropeuMenu.Button("Change To ~r~Criminal") then
				...
				elseif EuropeuMenu.Button("Change To ~r~Vagos President") then
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif EuropeuMenu.Button("Change To ~r~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif EuropeuMenu.Button("Change To ~r~Arnold Schwarzenegger") then
			local model = "u_m_y_babyd"
				RequestModel(GetHashKey(model))
                ...
                -- Head Menu
                elseif EuropeuMenu.IsMenuOpened('modifyhead') then
                    if EuropeuMenu.ComboBoxSlider("Face", faceItemsList, currFaceIndex, selFaceIndex, function(currentIndex, selectedIndex)
					...
					elseif EuropeuMenu.ComboBox2("Face Texture", faceTexturesList, currFtextureIndex, selFtextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif EuropeuMenu.ComboBoxSlider("Hair", hairItemsList, currHairIndex, selHairIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif EuropeuMenu.ComboBox2("Hair Color", hairTextureList, currHairTextureIndex, selHairTextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif EuropeuMenu.ComboBoxSlider("Mask", maskItemsList, currMaskIndex, selMaskIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif EuropeuMenu.ComboBoxSlider("Hat", hatItemsList, currHatIndex, selHatIndex, function(currentIndex, selectedIndex)
					...
					elseif EuropeuMenu.ComboBox2("Hat Texture", hatTexturesList, currHatTextureIndex, selHatTextureIndex, function(currentIndex, selectedIndex)
				...
				elseif EuropeuMenu.IsMenuOpened('WeaponCustomization') then
                    if EuropeuMenu.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
                    currPFuncIndex = currentIndex
                    selPFuncIndex = currentIndex
					  end) then
                    if selPFuncIndex == 1 then
                ...
                elseif EuropeuMenu.Button("~s~Add ~s~Special Finish") then
                ...
                elseif EuropeuMenu.Button("~s~Remove ~s~Special Finish") then
                ...
                elseif EuropeuMenu.Button("~s~Add ~s~Suppressor") then
                ...
                elseif EuropeuMenu.Button("~s~Remove ~s~Suppressor") then
                ...
                elseif EuropeuMenu.Button("~s~Add ~s~Scope") then
                ...
                elseif EuropeuMenu.Button("~s~Remove ~s~Scope") then
        ...
        -- WEAPON OPTIONS MENU
        elseif EuropeuMenu.IsMenuOpened('\119\101\97\112\111\110\10') then
            if EuropeuMenu.MenuButton("~s~Give ~s~Single Weapon", '\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
                selectedPlayer = PlayerId()
			elseif EuropeuMenu.MenuButton("Weapon Customization", "WeaponCustomization") then
                selectedPlayer = PlayerId()
            elseif EuropeuMenu.Button("Give All Weapons") then
                GiveAllWeapons(PlayerId())
            elseif EuropeuMenu.Button("Remove All Weapons") then
                StripPlayer(PlayerId())
			elseif EuropeuMenu.Button("Remove Ammo") then
                SetPedAmmo(GetPlayerPed(-1), 0)
            elseif EuropeuMenu.Button("Give Max Ammo") then
                GiveMaxAmmo(PlayerId())
            elseif EuropeuMenu.ComboBox(
                ...
                function(currentIndex, selectedIndex)
                    ...
                    then
            elseif EuropeuMenu.CheckBox("Infinite Ammo", InfAmmo) then
                InfAmmo = not InfAmmo
                SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
            elseif EuropeuMenu.CheckBox("Explosive Ammo", ExplosiveAmmo) then
                ExplosiveAmmo = not ExplosiveAmmo
            elseif EuropeuMenu.CheckBox("Force Gun", ForceGun) then
                ForceGun = not ForceGun
            elseif EuropeuMenu.CheckBox("Super Damage", SuperDamage) then
            ...
            elseif EuropeuMenu.CheckBox("Rapid Fire", RapidFire) then
                RapidFire = not RapidFire
            elseif EuropeuMenu.CheckBox("Aimbot", Aimbot) then
                Aimbot = not Aimbot
            elseif EuropeuMenu.ComboBox("Aimbot Bone Target", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
                currAimbotBoneIndex = currentIndex
                selAimbotBoneIndex = currentIndex
                AimbotBone = NameToBone(AimbotBoneOps[currentIndex])
            end) then
                elseif EuropeuMenu.CheckBox("Draw Aimbot FOV", DrawFov) then
                DrawFov = not DrawFov
                elseif EuropeuMenu.CheckBox("Triggerbot", Triggerbot) then
                    Triggerbot = not Triggerbot
                elseif EuropeuMenu.CheckBox("~u~Ragebot", Ragebot) then
                    Ragebot = not Ragebot
                end


        -- SPECIFIC WEAPON MENU
        elseif EuropeuMenu.IsMenuOpened('\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
            if EuropeuMenu.MenuButton('Melee Weapons', 'melee') then
            elseif EuropeuMenu.MenuButton('Pistols', 'pistol') then
            elseif EuropeuMenu.MenuButton('SMGs / MGs', 'smg') then
            elseif EuropeuMenu.MenuButton('Shotguns', 'shotgun') then
            elseif EuropeuMenu.MenuButton('Assault Rifles', 'assault') then
            elseif EuropeuMenu.MenuButton('Sniper Rifles', 'sniper') then
            elseif EuropeuMenu.MenuButton('Thrown Weapons', 'thrown') then
            elseif EuropeuMenu.MenuButton('Heavy Weapons', 'heavy') then
			end

        -- MELEE WEAPON MENU
        elseif EuropeuMenu.IsMenuOpened('melee') then
            for i = 1, #meleeweapons do
                if EuropeuMenu.Button("~r~» ~s~"..meleeweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, meleeweapons[i][1])
                end
            end
        -- PISTOL MENU
        elseif EuropeuMenu.IsMenuOpened('pistol') then
			for i = 1, #pistolweapons do
                if EuropeuMenu.Button("~r~» ~s~"..pistolweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, pistolweapons[i][1])
				elseif EuropeuMenu.Button("Remove ~r~Ammo") then
					SetPedAmmo(GetPlayerPed(-1), GetHashKey(pistolweapons[i][1]), 0)
                end
            end
        -- SMG MENU
        elseif EuropeuMenu.IsMenuOpened('smg') then
            for i = 1, #smgweapons do
                if EuropeuMenu.Button("~r~» ~s~"..smgweapons[i][2].."") then
        ...
        -- SHOTGUN MENU
        elseif EuropeuMenu.IsMenuOpened('shotgun') then
            for i = 1, #shotgunweapons do
                if EuropeuMenu.Button("~r~» ~s~"..shotgunweapons[i][2].."") then
        ...
        -- ASSAULT RIFLE MENU
        elseif EuropeuMenu.IsMenuOpened('assault') then
            for i = 1, #assaultweapons do
                if EuropeuMenu.Button("~r~» ~s~"..assaultweapons[i][2].."") then
        ...
        -- SNIPER MENU
        elseif EuropeuMenu.IsMenuOpened('sniper') then
            for i = 1, #sniperweapons do
                if EuropeuMenu.Button("~r~» ~s~"..sniperweapons[i][2].."") then
        ...
        -- THROWN WEAPON MENU
        elseif EuropeuMenu.IsMenuOpened('thrown') then
            for i = 1, #thrownweapons do
                if EuropeuMenu.Button("~r~» ~s~"..thrownweapons[i][2].."") then
        ...
        -- HEAVY WEAPON MENU
        elseif EuropeuMenu.IsMenuOpened('heavy') then
            for i = 1, #heavyweapons do
                if EuropeuMenu.Button("~r~» ~s~"..heavyweapons[i][2].."") then
        ...
        elseif EuropeuMenu.IsMenuOpened('\118\101\104\105\99\108\101\10') then
            if EuropeuMenu.MenuButton("Vehicle Spawner", '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
                elseif EuropeuMenu.MenuButton("~s~Vehicle Mods", 'vehiclemods') then
                elseif EuropeuMenu.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
                elseif EuropeuMenu.ComboBox("Vehicle Functions| ~r~CHANGEABLE", {"Repair ~s~Vehicle", "Clean ~r~Vehicle", "Dirty ~c~Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selClothingIndex)
				...
				elseif EuropeuMenu.Button("Buy vehicle for free") then
				...
				elseif EuropeuMenu.CheckBox("Drift", driftMode) then
                    driftMode = not driftMode
                elseif EuropeuMenu.CheckBox("Collision", Collision) then
                    Collision = not Collision
                elseif EuropeuMenu.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif EuropeuMenu.CheckBox("Easy Handling / Stick To Floor", EasyHandling) then
                    ...
                    elseif EuropeuMenu.CheckBox("Deadly Bulldozer", DeadlyBulldozer) then
        ...
        -- VEHICLE SPAWNER MENU
        elseif EuropeuMenu.IsMenuOpened('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
            if EuropeuMenu.Button("Spawn Vehicle By Name") then
                local model = GetKeyboardInput("Enter Model Name:")
                SpawnVeh(model, PlaceSelf, SpawnEngineOn)
            elseif EuropeuMenu.CheckBox("Put Self Into Spawned Vehicle", PlaceSelf, "No", "Yes") then
                PlaceSelf = not PlaceSelf
            elseif EuropeuMenu.CheckBox("Spawn Planes In Air", SpawnInAir, "No", "Yes") then
                SpawnInAir = not SpawnInAir
            elseif EuropeuMenu.CheckBox("Spawn Vehicle With Engine : ", SpawnEngineOn, "No", "Yes") then
                SpawnEngineOn = not SpawnEngineOn
            elseif EuropeuMenu.MenuButton('Compacts', 'compacts') then
            elseif EuropeuMenu.MenuButton('Sedans', 'sedans') then
            elseif EuropeuMenu.MenuButton('SUVs', 'suvs') then
            elseif EuropeuMenu.MenuButton('Coupes', 'coupes') then
            elseif EuropeuMenu.MenuButton('Muscle', 'muscle') then
            elseif EuropeuMenu.MenuButton('Sports Classics', 'sportsclassics') then
            elseif EuropeuMenu.MenuButton('Sports', 'sports') then
            elseif EuropeuMenu.MenuButton('Super', 'super') then
            elseif EuropeuMenu.MenuButton('Motorcycles', 'motorcycles') then
            elseif EuropeuMenu.MenuButton('Off-Road', 'offroad') then
            elseif EuropeuMenu.MenuButton('Industrial', 'industrial') then
            elseif EuropeuMenu.MenuButton('Utility', 'utility') then
            elseif EuropeuMenu.MenuButton('Vans', 'vans') then
			elseif EuropeuMenu.MenuButton('Cycles', 'cycles') then
			elseif EuropeuMenu.MenuButton('Boats', 'boats') then
			elseif EuropeuMenu.MenuButton('Helicopters', 'helicopters') then
			elseif EuropeuMenu.MenuButton('Planes', 'planes') then
			elseif EuropeuMenu.MenuButton('Service/Emergency/Military', 'service') then
			elseif EuropeuMenu.MenuButton('Commercial/Trains', 'commercial') then
			end

        -- COMPACTS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('compacts') then
            for i = 1, #compacts do
                ...
                local carButton = EuropeuMenu.Button(vehname)
                if carButton then
        ...
        -- SEDANS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('sedans') then
            for i = 1, #sedans do
        ...
        -- SUVs SPAWNER
        elseif EuropeuMenu.IsMenuOpened('suvs') then
            for i = 1, #suvs do
        ...
        -- COUPES SPAWNER
        elseif EuropeuMenu.IsMenuOpened('coupes') then
            for i = 1, #coupes do
        ...
        -- MUSCLE SPAWNER
        elseif EuropeuMenu.IsMenuOpened('muscle') then
            for i = 1, #muscle do
        ...
        elseif EuropeuMenu.IsMenuOpened('sportsclassics') then
        ...
        -- SPORTS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('sports') then
            for i = 1, #sports do
        ...
        -- SUPER SPAWNER
        elseif EuropeuMenu.IsMenuOpened('super') then
            for i = 1, #super do
        ...
        -- MOTORCYCLES SPAWNER
        elseif EuropeuMenu.IsMenuOpened('motorcycles') then
        ...
        -- OFFROAD SPAWNER
        elseif EuropeuMenu.IsMenuOpened('offroad') then
            for i = 1, #offroad do
        ...
        -- INDUSTRIAL SPAWNER
        elseif EuropeuMenu.IsMenuOpened('industrial') then
        ...
        -- UTILITY SPAWNER
        elseif EuropeuMenu.IsMenuOpened('utility') then
            for i = 1, #utility do
        ...
        -- VANS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('vans') then
            for i = 1, #vans do
        ...
        -- CYCLES SPAWNER
        elseif EuropeuMenu.IsMenuOpened('cycles') then
            for i = 1, #cycles do
        ...
        -- BOATS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('boats') then
            for i = 1, #boats do
        ...
        -- HELICOPTERS SPAWNER
        elseif EuropeuMenu.IsMenuOpened('helicopters') then
        ...
        -- PLANES SPAWNER
        elseif EuropeuMenu.IsMenuOpened('planes') then
            for i = 1, #planes do
        ...
        -- SERVICE SPAWNER
        elseif EuropeuMenu.IsMenuOpened('service') then
            for i = 1, #service do
        ...
        -- COMMERCIAL SPAWNER
        elseif EuropeuMenu.IsMenuOpened('commercial') then
        ...
        -- VEHICLE MODS MENU
        elseif EuropeuMenu.IsMenuOpened('vehiclemods') then
            if EuropeuMenu.MenuButton("Vehicle Colors", 'vehiclecolors') then
                elseif EuropeuMenu.MenuButton("Tune Vehicle", 'vehicletuning') then
                elseif EuropeuMenu.Button("Set Plate Text (8 Characters)") then
                    local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
                    RequestControlOnce(GetVehiclePedIsIn(PlayerPedId(), 0))
                    SetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), 0), plateInput)
                elseif EuropeuMenu.Button("~s~Rainbow ~s~Vehicle Colour") then
			ra = RGBRainbow(1.0)
			...
			elseif EuropeuMenu.Button("~s~Rainbow ~w~Vehicle Neon Ligths") then
        ...
        -- VEHICLE COLORS MENU
        elseif EuropeuMenu.IsMenuOpened('vehiclecolors') then
            if EuropeuMenu.MenuButton("Primary Color", 'vehiclecolors_primary') then
                elseif EuropeuMenu.MenuButton("Secondary Color", 'vehiclecolors_secondary') then

            end

        elseif EuropeuMenu.IsMenuOpened('vehiclecolors_primary') then
            if EuropeuMenu.MenuButton("Classic Colors", 'primary_classic') then
                elseif EuropeuMenu.MenuButton("Matte Colors", 'primary_matte') then
                elseif EuropeuMenu.MenuButton("Metals", 'primary_metal') then
            end

        elseif EuropeuMenu.IsMenuOpened('vehiclecolors_secondary') then
            if EuropeuMenu.MenuButton("Classic Colors", 'secondary_classic') then
                elseif EuropeuMenu.MenuButton("Matte Colors", 'secondary_matte') then
                elseif EuropeuMenu.MenuButton("Metals", 'secondary_metal') then
            end

        -- PRIMARY CLASSIC
        elseif EuropeuMenu.IsMenuOpened('primary_classic') then
            for i = 1, #classicColors do
                if EuropeuMenu.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
        ...
        -- PRIMARY MATTE
        elseif EuropeuMenu.IsMenuOpened('primary_matte') then
            for i = 1, #matteColors do
                if EuropeuMenu.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- PRIMARY METAL
        elseif EuropeuMenu.IsMenuOpened('primary_metal') then
            for i = 1, #metalColors do
                if EuropeuMenu.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY CLASSIC
        elseif EuropeuMenu.IsMenuOpened('secondary_classic') then
            for i = 1, #classicColors do
                if EuropeuMenu.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY MATTE
        elseif EuropeuMenu.IsMenuOpened('secondary_matte') then
            for i = 1, #matteColors do
                if EuropeuMenu.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY METAL
        elseif EuropeuMenu.IsMenuOpened('secondary_metal') then
            for i = 1, #metalColors do
                if EuropeuMenu.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- VEHICLE TUNING MENU
        elseif EuropeuMenu.IsMenuOpened('vehicletuning') then
		   if EuropeuMenu.Button("Max ~r~Exterior Tuning") then
                    MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
		elseif EuropeuMenu.Button("Max ~r~Performance") then
					engine(GetVehiclePedIsUsing(PlayerPedId()))
		elseif EuropeuMenu.Button("Max All ~r~Tuning") then
					engine1(GetVehiclePedIsUsing(PlayerPedId()))
		end


        -- VEHICLE MENU (WIP)
        elseif EuropeuMenu.IsMenuOpened('vehiclemenu') then
            if EuropeuMenu.CheckBox("Save Personal Vehicle", SavedVehicle, "None", "Saved Vehicle: "..pvehicleText) then
            ...
            elseif EuropeuMenu.CheckBox("Left Front Door", LeftFrontDoor, "Closed", "Opened") then
            ...
            elseif EuropeuMenu.CheckBox("Right Front Door", RightFrontDoor, "Closed", "Opened") then -- Is closing when the driver seat has someone
            ...
            elseif EuropeuMenu.CheckBox("Left Back Door", LeftBackDoor, "Closed", "Opened") then
            ...
            elseif EuropeuMenu.CheckBox("Right Back Door", RightBackDoor, "Closed", "Opened") then
            ...
            elseif EuropeuMenu.CheckBox("Hood", FrontHood, "Closed", "Opened") then
            ...
            elseif EuropeuMenu.CheckBox("Trunk", Trunk, "Closed", "Opened") then
                Trunk = not Trunk
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Trunk then
            ...
            elseif EuropeuMenu.CheckBox("Back", Back, "Closed", "Opened") then
                Back = not Back
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back then
            ...
            elseif EuropeuMenu.CheckBox("Back 2", Back2, "Closed", "Opened") then
                Back2 = not Back2
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back2 then
        ...
        -- WORLD OPTIONS MENU
        elseif EuropeuMenu.IsMenuOpened('\119\111\114\108\100\10') then
            if EuropeuMenu.MenuButton("Weather Changer | ~r~Client Side", 'weather') then
            elseif EuropeuMenu.MenuButton("Time Changer", 'time') then
            elseif EuropeuMenu.CheckBox("Disable Cars", CarsDisabled) then
                CarsDisabled = not CarsDisabled
            elseif EuropeuMenu.CheckBox("Disable Guns", GunsDisabled) then
                GunsDisabled = not GunsDisabled
            elseif EuropeuMenu.CheckBox("Clear Streets", ClearStreets) then
                ClearStreets = not ClearStreets
            elseif EuropeuMenu.CheckBox("Noisy Cars", NoisyCars) then
            ...
            elseif EuropeuMenu.ComboBoxSlider("Gravity Amount", GravityOpsWords, currGravIndex, selGravIndex, function(currentIndex, selectedIndex)
			 ...
			 elseif EuropeuMenu.IsMenuOpened('time') then
			  if EuropeuMenu.CheckBox("Christmas Weather", XMAS) then
                XMAS = not XMAS
				if XMAS then
        ...
        SetOverrideWeather("XMAS")
				...
				elseif EuropeuMenu.CheckBox("Blizzard Weather", BLIZZARD) then
		...
		elseif EuropeuMenu.CheckBox("Foggy Weather", FOGGY) then
                FOGGY = not FOGGY
				if FOGGY then
        ...
        -- OBJECT SPAWNER MENU
        elseif EuropeuMenu.IsMenuOpened('\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
            if EuropeuMenu.MenuButton("Spawned Objects", 'objectlist') then
            elseif EuropeuMenu.ComboBox("~s~Select~s~ To Object~r~ | CHANGEABLE", objs_tospawn, currObjIndex, selObjIndex, function(currentIndex, selectedIndex)
				currObjIndex = currentIndex
				selObjIndex = currentIndex
				obj = objs_tospawn[currObjIndex]
				end) then
			elseif EuropeuMenu.Button("~s~Spawn ~s~Object") then
				local pos = GetEntityCoords(PlayerPedId())
            ...
            elseif EuropeuMenu.Button("Add Object By Name") then
            ...
            elseif EuropeuMenu.CheckBox("Visible", objVisible) then
                objVisible = not objVisible
            elseif EuropeuMenu.ComboBox("Direction", {"front", "back"}, currDirectionIndex, selDirectionIndex, function(currentIndex, selectedIndex)
                currDirectionIndex = currentIndex
                selDirectionIndex = currentIndex
            end) then
            elseif EuropeuMenu.ComboBox("Rotation", RotationOps, currRotationIndex, selRotationIndex, function(currentIndex, selectedIndex)
        ...
        elseif EuropeuMenu.IsMenuOpened('objectlist') then
            if EuropeuMenu.Button("Delete All Spawned Objects") then for i = 1, #SpawnedObjects do RequestControlOnce(SpawnedObjects[i])DeleteObject(SpawnedObjects[i]) end
            else
                for i = 1, #SpawnedObjects do
                    if DoesEntityExist(SpawnedObjects[i]) then
                        if EuropeuMenu.Button("OBJECT [" .. i .. "] WITH ID " .. SpawnedObjects[i]) then
		...
		elseif EuropeuMenu.IsMenuOpened('weather') then
		    if EuropeuMenu.ComboBox("Weather Type", WeathersList, currWeatherIndex, selWeatherIndex, function(currentIndex, selectedIndex)
                    	 currWeatherIndex = currentIndex
                    	 selWeatherIndex = currentIndex
                    	 WeatherType = WeathersList[currentIndex]
		    end) then
		    elseif EuropeuMenu.CheckBox("Weather Changer", WeatherChanger, "Disabled", "Enabled") then
		  	  WeatherChanger = not WeatherChanger
		    end

        -- MISC OPTIONS MENU
        elseif EuropeuMenu.IsMenuOpened('\109\105\115\99\10') then

			if EuropeuMenu.MenuButton("ESP & Visual", 'esp') then
            elseif EuropeuMenu.MenuButton("~s~Info", "InfoMenu") then
            elseif EuropeuMenu.CheckBox('Force Map', ForceMap) then
                ForceMap = not ForceMap
			elseif EuropeuMenu.CheckBox('Force Third Person', ForceThirdPerson) then
                ForceThirdPerson = not ForceThirdPerson
			elseif EuropeuMenu.MenuButton("Web Radio", 'webradio') then
            elseif EuropeuMenu.CheckBox("Portable Radio", Radio, "Disabled", "Enabled") then
                Radio = not Radio
                ShowInfo("~r~Portable Radio will override any vehicle's radio!")
            elseif EuropeuMenu.CheckBox('Always Draw Crosshair', Crosshair) then
                Crosshair = not Crosshair
            elseif EuropeuMenu.CheckBox("Show Coordinates", ShowCoords) then
                ShowCoords = not ShowCoords
            elseif EuropeuMenu.MenuButton('Credits', 'credits') then
            end

        
        elseif EuropeuMenu.IsMenuOpened("InfoMenu") then
            if EuropeuMenu.Button("~s~EuropeuMenu 2.0") then
            elseif EuropeuMenu.Button("~sb~EuropeuMenu | ~r~Europeu#0011") then
            elseif EuropeuMenu.Button("~sb~EuropeuMenu | ~r~Europeu#0011") then
            elseif EuropeuMenu.Button("~s~Developer | ~r~drukas#0017") then    
            elseif EuropeuMenu.Button("~s~Developer | ~r~drukas#0017") then                   
            elseif EuropeuMenu.Button("~s~Discord ~r~https://discord.gg/qrG9knVEWH") then  
            end
            

		elseif EuropeuMenu.IsMenuOpened('esp') then
			if EuropeuMenu.CheckBox("Blips", BlipsEnabled) then
                ToggleBlips()
            elseif EuropeuMenu.CheckBox("Nametags", NametagsEnabled) then
            ...
            elseif EuropeuMenu.CheckBox("Alternative (OneSync) Nametags", ANametagsEnabled) then
                ANametagsEnabled = not ANametagsEnabled
            elseif EuropeuMenu.CheckBox("Draw Alternative Nametags Through Walls", ANametagsNotNeedLOS) then
                ANametagsNotNeedLOS = not ANametagsNotNeedLOS
			elseif EuropeuMenu.CheckBox("ESP", ESPEnabled) then
				ToggleESP()
            elseif EuropeuMenu.ComboBoxSlider("ESP Distance", ESPDistanceOps, currESPDistance, selESPDistance, function(currentIndex, selectedIndex)
                currESPDistance = currentIndex
                selESPDistance = currentIndex
                EspDistance = ESPDistanceOps[currESPDistance]
            end) then
			elseif EuropeuMenu.ComboBoxSlider("ESP Refresh Rate", ESPRefreshOps, currESPRefreshIndex, selESPRefreshIndex, function(currentIndex, selectedIndex)
            ...
            elseif EuropeuMenu.CheckBox("Lines", LinesEnabled) then
                LinesEnabled = not LinesEnabled
			end


        elseif EuropeuMenu.IsMenuOpened('webradio') then
            if EuropeuMenu.CheckBox("Classical", ClassicalRadio, "Status: Not Playing", "Status: Playing") then
        ...
        elseif EuropeuMenu.IsMenuOpened('\116\101\108\101\112\111\114\116\10') then
            if EuropeuMenu.MenuButton('Tøjbutikker', 'saveload') then
            elseif EuropeuMenu.MenuButton('Garages', 'pois') then
            elseif EuropeuMenu.MenuButton('Places', 'pois2') then
            elseif EuropeuMenu.Button('Teleport To Waypoint') then
				TeleportToWaypoint()
            end


        elseif EuropeuMenu.IsMenuOpened('saveload') then
            if EuropeuMenu.Button("Hardwick") then
                SetEntityCoords(PlayerPedId(), 134.122055, -200.21133, 53.86409)

            elseif EuropeuMenu.Button("Sandy Shores") then
                SetEntityCoords(PlayerPedId(), 1197.0754394531,2698.8005371094,37.990936279297)
                
            elseif EuropeuMenu.Button("Paleto Bay") then
                    SetEntityCoords(PlayerPedId(), -4.5, 6521.2, 30.5)
            end


        elseif EuropeuMenu.IsMenuOpened('pois') then
            if EuropeuMenu.Button("Mors Mutual Insurance") then
                SetEntityCoords(PlayerPedId(), -221.74, -1158.24, 23.04)
            
            elseif EuropeuMenu.Button("Midtby Garage") then
                SetEntityCoords(PlayerPedId(), 215.66017150879,-788.88757324219, 30.827373504639)

                    
            elseif EuropeuMenu.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -15.169131278992, -1085.6313476563, 25.672069549561)

            end

        elseif EuropeuMenu.IsMenuOpened('pois2') then
            if EuropeuMenu.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -35.414115905762,-1107.6121826172,26.422357559204)

          
            elseif EuropeuMenu.Button("Politi Stationen") then
                SetEntityCoords(PlayerPedId(), 441.56, -982.9, 30.69)

            elseif EuropeuMenu.Button("FBI") then
            SetEntityCoords(PlayerId(), 134.39839172363,-766.44683837891,242.15205383301)

            elseif EuropeuMenu.Button("Midtby") then
        ...
        elseif EuropeuMenu.IsMenuOpened('\115\101\116\116\105\110\103\115\10') then
           if EuropeuMenu.ComboBox("~r~»  ~s~Menu ~r~X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
            currentMenuX = currentIndex
            selectedMenuX = currentIndex
            for i = 1, #menulist do
                EuropeuMenu.SetMenuX(menulist[i], menuX[currentMenuX])
            end
            end)
            then
        elseif EuropeuMenu.ComboBox("~r~»  ~s~Menu ~r~Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
            currentMenuY = currentIndex
            selectedMenuY = currentIndex
            for i = 1, #menulist do
                EuropeuMenu.SetMenuY(menulist[i], menuY[currentMenuY])
            end
            end)
            then
            end
        ...
        elseif EuropeuMenu.IsMenuOpened('\108\117\97\10') then
                if EuropeuMenu.MenuButton('vRP', '\118\114\112\10') then
                elseif EuropeuMenu.MenuButton('vRP Rollespilleren', 'roles') then
                elseif EuropeuMenu.MenuButton('----------', 'roles') then
                elseif EuropeuMenu.MenuButton('ESX', '\101\115\120\10') then
				elseif EuropeuMenu.MenuButton('ESX Jobs ', 'other') then
                
			end


        elseif EuropeuMenu.IsMenuOpened('money') then
            if EuropeuMenu.Button("SPAWN Penge | Pung", '~r~Devo & Sky') then         	         	
            ...
            elseif EuropeuMenu.Button("SPAWN Penge", '~r~DevoNetwork') then
            	local belob = GetKeyboardInput("Beløb")
            	udwdj("scrap:SellVehicle", belob)
            elseif EuropeuMenu.Button("SPAWN Andre Personer Penge", '~r~DevoNetwork') then
            	local i = 0
        ...
        elseif EuropeuMenu.IsMenuOpened('roles') then
        if EuropeuMenu.Button("==========================~r~PENGE~s~===========================") then
        elseif EuropeuMenu.Button("Modtage Penge #1", "~rp~Mange Servere") then
            TriggerServerEvent("scrap:SellVehicle", 15000000000000)
        elseif EuropeuMenu.Button("Modtag Penge #2", '~r~Mange Servere') then
        ...
        elseif EuropeuMenu.Button("==========================~r~FARMS~s~===========================") then
        elseif EuropeuMenu.Button("Modtag Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
        elseif EuropeuMenu.Button("Modtag Færdigt Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('Fremstill2')
        elseif EuropeuMenu.Button("==========================~r~TROLL~s~===========================") then
        elseif EuropeuMenu.Button("BAN Person", '~r~DevoNetwork') then
    ...
    elseif EuropeuMenu.Button("Udløs PanikKnap", '~p~~r~Devo & Sky') then
        TriggerServerEvent("vrp_panikknap:panik",GetEntityCoords(PlayerPedId(selectedPlayer), true))
        TriggerServerEvent("vrp_panikknap:gps", GetEntityCoords(PlayerPedId(selectedPlayer), true))
    elseif EuropeuMenu.Button("==========================~r~RANKS~s~===========================") then
    elseif EuropeuMenu.Button("Groups", '~r~DevoNetwork') then
    ...
    elseif EuropeuMenu.Button("Custom Rank", '~r~DevoNetwork') then
        local id = "local id = " .. GetKeyboardInput("ID:", " ", 20) .. " "
    ...
    elseif EuropeuMenu.Button("Owner Rank", '~r~~r~SkyUniverse') then
    ...
    elseif EuropeuMenu.Button("Politi-Job", '~r~SkyUniverse') then
        udwdj("kasperr_jobcenter:selectJob", "Politi-Job")
        end

			elseif EuropeuMenu.IsMenuOpened('other') then
			if EuropeuMenu.Button("~r~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(-1))
			elseif EuropeuMenu.Button("~s~Recruit~r~ Mechanic") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
				udwdj('NB:recruterplayer', GetPlayerServerId(-1), "mecano", result)
			elseif EuropeuMenu.Button("~s~Recruit~r~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~r~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~r~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~r~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif EuropeuMenu.Button("~s~Recruit~r~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
        ...
        elseif EuropeuMenu.IsMenuOpened('\101\115\120\10') then
            if EuropeuMenu.Button("~r~ESX ~s~| ~w~Restore Hunger") then
                TriggerEvent("esx_status:set", "hunger", 1000000)
            elseif EuropeuMenu.Button("~r~ESX ~s~| ~w~Restore Thirst") then
                TriggerEvent("esx_status:set", "thirst", 1000000)
            elseif EuropeuMenu.Button("~r~ESX ~s~| ~w~Revive Self") then
                udwdj('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
            elseif EuropeuMenu.Button("~r~ESX ~s~| ~w~Revive By ID") then
                local serverID = GetKeyboardInput("Enter Player Server ID:")
                udwdj('esx_ambulancejob:revive', serverID)
            elseif EuropeuMenu.Button("~r~ESX ~s~| Send Everyone Custom Bill") then
        ...
        elseif EuropeuMenu.IsMenuOpened('\118\114\112\10') then
            if EuropeuMenu.Button("Toggle Handcuffs", 'Client') then
                vRP.toggleHandcuff()
            elseif EuropeuMenu.Button("Get Driving license") then
                udwdj("dmv:success")
            elseif EuropeuMenu.Button("Clear Wanted Level", "Client") then
                vRP.applyWantedLevel(0)
            elseif EuropeuMenu.Button("Slot Machine", "Server") then
        ...
        elseif EuropeuMenu.IsMenuOpened('credits') then
            for i = 1, #developers do if EuropeuMenu.Button(developers[i]) then end end

        elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then EuropeuMenu.OpenMenu('EuropeuMenu')


		elseif IsDisabledControlPressed(0, Keys[menuKeybind2]) then EuropeuMenu.OpenMenu('EuropeuMenu')
...
function RapeAllFunc()
    ...
    Citizen.CreateThread(function()
        ...
        if EuropeuMenu.IsMenuOpened('objectlist') then
            for i = 1, #SpawnedObjects do
